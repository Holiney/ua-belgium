...
const Task3PrintRooms = ({ lang }) => {
  ...
  const sync = async () => {
    ...
    if (!window.confirm(`Google Sync: ${room}?`)) return;
    setSyncing(true);
    try {
...
const HistoryView = ({ lang }) => {
  const [dates] = useState(getAllSavedDates());
  return (
    <div className="space-y-6">
      ...
      {!dates.length ? (
        ...
        <div className="space-y-4">
          {dates.map((d) => (
            <div
              key={d}
              className="rounded-2xl bg-white dark:bg-gray-800 shadow p-6 border border-gray-100 dark:border-gray-700"
            >
